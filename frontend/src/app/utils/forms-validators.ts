...
export class FormsValidators {
  public static getPasswordStrength(password: string, rules: any = null): number {
    let strength = 0;
    ...
    if(rules){
      if (password.length >= 8) {
        rules.find(t => t.type == 0).success = true;
      }
      else{
        rules.find(t => t.type == 0).success = false;
      }

      if (password.match(/[a-z]+/)) {
        rules.find(t => t.type == 1).success = true;
      }
      else{
        rules.find(t => t.type == 1).success = false;
      }

      if (password.match(/[A-Z]+/)) {
        rules.find(t => t.type == 2).success = true;
      }
      else{
        rules.find(t => t.type == 2).success = false;
      }

      if (password.match(/[0-9]+/)) {
        rules.find(t => t.type == 3).success = true;
      }
      else{
        rules.find(t => t.type == 3).success = false;
      }

      if (password.match(/[\W]+/)) {
        rules.find(t => t.type == 4).success = true;
      }
      else{
        rules.find(t => t.type == 4).success = false;
      }
    }
  ...
  public static passwordTooWeak(input: FormControl) {
    if (input.value) {
      ...
      return strength < 5 ? { weakPassword: true } : null;
    }
    return null;
  }
